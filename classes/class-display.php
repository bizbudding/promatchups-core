...
class ProMatchups_Display {
	...
	function set_vars() {
		...
		$access = pm_has_pro_access( $league );
	...
	function matchup_time_shortcode( $atts ) {
		...
		return pm_get_matchup_datetime( get_the_ID(), $atts['before'], $atts['after'] );
	}
	...
	function matchup_teams_shortcode( $atts ) {
		return pm_get_matchup_teams_list( $atts );
	}
