...
class ProMatchups_Endpoints {
	...
	/**
	 * Handle the insight request.
	 *
	 * @since 0.1.0
	 *
	 * @return WP_REST_Response|WP_Error
	 */
	function handle_insight_request( $request ) {
		$listener = new ProMatchups_AskNews_Insight_Listener( $request->get_body(), $this->user );
	...
	function handle_outcome_request( $request ) {
		$listener = new ProMatchups_AskNews_Outcome_Listener( $request->get_body(), $this->user );
		$response = $listener->get_response();
