...
/**
 * Get the matchup data, including team names, outcome, prediction, odds, spreads, etc.
 *
 * @since 0.8.0
 * @since 1.0.0 Now uses ProMatchups_Matchup_Data class.
 *
 * @param int $matchup_id The matchup post ID.
 *
 * @return array
 */
function pm_get_matchup_data( $matchup_id ) {
	return ProMatchups_Matchup_Data::get( $matchup_id );
}

/**
 * Get the matchup content.
 *
 * @since 1.0.0
 *
 * @param int $matchup_id The matchup post ID.
 ...
 * @return array
 */
function pm_get_matchup_content( $matchup_id ) {
	return ProMatchups_Matchup_Content::get( $matchup_id );
}
...
//  * @return string
//  */
// function pm_get_matchup_league( $matchup_id ) {
// 	static $cache = [];
 ...
 * @return array
 */
function pm_get_insight_body( $matchup ) {
	static $cache = [];

	// Maybe return cache.
	if ( isset( $cache[ $matchup ] ) ) {
		return $cache[ $matchup ];
	}

	// Set vars.
	$insight_id = pm_get_insight_id( $matchup );
	$temp       = $insight_id ? (array) get_post_meta( $insight_id, 'asknews_body', true ) : [];

	// Set cache.
	$cache[ $matchup ]    = $temp;
	$cache[ $insight_id ] = $temp;
 ...
 * @return int|null
 */
function pm_get_insight_id( $matchup ) {
