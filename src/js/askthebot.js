...
document.addEventListener('DOMContentLoaded', () => {
	...
	form.addEventListener('submit', async (event) => {
		...
		const imgString  = '<img src="/wp-content/plugins/promatchups-core/src/img/chad-head.png" alt="robot head with headset" />';
		...
		.then( data => {
			...
			// If we have a chat ID.
			if ( data.data.chatId ) {
				// Add it as a query arg, updating the old one if it exists.
				const url = new URL( window.location.href );
				url.searchParams.set( 'chat', data.data.chatId );
				window.history.replaceState( null, '', url );

				// Update the hidden field value.
				document.querySelector('input[name="askthebot_post_id"]').value = data.data.chatId;
			}
